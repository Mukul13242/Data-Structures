#include <iostream>           //Name:Mukul Goyal          
using namespace std;         //ECE SID:21105071
...
int binary_search(int arr[],int i,int j,int key){
    ...
    while(i<=j){   //i=starting index and j=ending index
...
//Enhanced binary search function for unknown length
int binary_search_unknown(int arr[],int key){
    //Initializing i as 0 and j as 1.
    int i=0,j=1;
    //We will increase j until arr[j] increases the search
    while(arr[j]<key){
        i=j;
        j=2*j;
    }
    //Now calling Simple Binary Search from range i to j.
